...
A `ModuleRegistration` in `backend/modules/registry/registration.py`:

| field | meaning |
|---|---|
| `id` | `M02` |
| `slug` | `registry` |
| `api_prefix` | `/api/registry/` |
| `frontend_routes` | React routes, each with nav metadata and required permission |
| `permission_codes` | all namespaced `registry.<verb>_<noun>` |
...
```bash
python scripts/dev.py up M02 --profile standalone
python scripts/dev.py migrate M02 --profile standalone
python scripts/dev.py seed M02 --scenario baseline
python scripts/dev.py check M02 --suite standalone
```
