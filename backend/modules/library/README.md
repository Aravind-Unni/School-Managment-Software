...
`import modules.library` does technically succeed and yields an empty module. What
does **not** exist -- and what the guarantee actually rests on -- is
`modules.library.registration`. Importing it raises `ModuleNotFoundError`, which is
why `scripts/dev.py up M09` reports "not implemented" instead of booting an empty
...
Read [`contracts/M09/PACKET.md`](../../../contracts/M09/PACKET.md) first. The
