...
`import modules.platform` does technically succeed and yields an empty module. What
does **not** exist -- and what the guarantee actually rests on -- is
`modules.platform.registration`. Importing it raises `ModuleNotFoundError`, which is
why `scripts/dev.py up M14` reports "not implemented" instead of booting an empty
...
Read [`contracts/M14/PACKET.md`](../../../contracts/M14/PACKET.md) first. The
