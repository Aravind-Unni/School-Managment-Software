...
`import modules.registry` does technically succeed and yields an empty module. What
does **not** exist -- and what the guarantee actually rests on -- is
`modules.registry.registration`. Importing it raises `ModuleNotFoundError`, which is
why `scripts/dev.py up M02` reports "not implemented" instead of booting an empty
...
Read [`contracts/M02/PACKET.md`](../../../contracts/M02/PACKET.md) first. The
