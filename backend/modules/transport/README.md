...
`import modules.transport` does technically succeed and yields an empty module. What
does **not** exist -- and what the guarantee actually rests on -- is
`modules.transport.registration`. Importing it raises `ModuleNotFoundError`, which is
why `scripts/dev.py up M08` reports "not implemented" instead of booting an empty
...
Read [`contracts/M08/PACKET.md`](../../../contracts/M08/PACKET.md) first. The
