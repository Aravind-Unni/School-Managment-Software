...
`import modules.access` does technically succeed and yields an empty module. What
does **not** exist -- and what the guarantee actually rests on -- is
`modules.access.registration`. Importing it raises `ModuleNotFoundError`, which is
why `scripts/dev.py up M01` reports "not implemented" instead of booting an empty
...
Read [`contracts/M01/PACKET.md`](../../../contracts/M01/PACKET.md) first. The
