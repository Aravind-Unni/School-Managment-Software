...
`import modules.timetable` does technically succeed and yields an empty module. What
does **not** exist -- and what the guarantee actually rests on -- is
`modules.timetable.registration`. Importing it raises `ModuleNotFoundError`, which is
why `scripts/dev.py up M03` reports "not implemented" instead of booting an empty
...
Read [`contracts/M03/PACKET.md`](../../../contracts/M03/PACKET.md) first. The
