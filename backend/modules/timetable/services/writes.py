"""Shared mechanics for M03's versioned, school-scoped writes.
...
Deliberately a copy of M02's equivalent rather than an import of it: a module may
not import another module, and four small functions repeated in two places is the
...
def fetch_in_school[ModelType: models.Model](
    ...
    Does not handle: withdrawn rows. Withdrawal hides a row from NEW effect, not
    from reads of history, so a withdrawn row is still returned.
    """
...
def require_expected_version(row: models.Model, expected_version: int) -> None:
    ...
    Assumes the row was fetched inside the same transaction, with a lock when the
    caller intends to write. Checking an unlocked row is a check against a value
    another transaction may already have moved.
    """
    current = row.version
...
def stamp_update(row: models.Model, *, now: datetime) -> models.Model:
    ...
    forgot to advance the version would make every subsequent stale write succeed,
    and the tests for that would still pass.
    """
