# backend/modules/performance -- M06
...
`import modules.performance` does technically succeed and yields an empty module. What
does **not** exist -- and what the guarantee actually rests on -- is
`modules.performance.registration`. Importing it raises `ModuleNotFoundError`, which is
why `scripts/dev.py up M06` reports "not implemented" instead of booting an empty
...
Read [`contracts/M06/PACKET.md`](../../../contracts/M06/PACKET.md) first. The
