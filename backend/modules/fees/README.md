...
`import modules.fees` does technically succeed and yields an empty module. What
does **not** exist -- and what the guarantee actually rests on -- is
`modules.fees.registration`. Importing it raises `ModuleNotFoundError`, which is
why `scripts/dev.py up M07` reports "not implemented" instead of booting an empty
...
Read [`contracts/M07/PACKET.md`](../../../contracts/M07/PACKET.md) first. The
